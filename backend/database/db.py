...
def _migrate_users_columns(conn: sqlite3.Connection) -> None:
    ...
    if "theme" not in existing:
        conn.execute(
            "ALTER TABLE users ADD COLUMN theme TEXT NOT NULL DEFAULT 'classic'"
        )
    conn.execute(
        "UPDATE users SET theme = 'classic' WHERE theme IS NULL OR TRIM(theme) = ''"
    )
...
def _migrate_messenger_columns(conn: sqlite3.Connection) -> None:
    ...
    if "brief_genre" not in existing:
        conn.execute(
            "ALTER TABLE messenger_contacts ADD COLUMN brief_genre TEXT NOT NULL DEFAULT ''"
        )
    if "tz_name" not in existing:
        conn.execute(
            "ALTER TABLE messenger_contacts ADD COLUMN tz_name TEXT NOT NULL DEFAULT ''"
        )
    conn.execute(
